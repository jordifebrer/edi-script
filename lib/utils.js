...
exports.getTagAttributes = function(key, liveTagsArr, dict, separators) {
  ...
  if (/>/i.test(key)) {
    ...
    if (cssPropStr) {
      attributes.css += liveTagsArr.join(' ') + separators.space + dict.html[key] + ' {' + cssPropStr + separators.newLine + '}' + separators.newLine + separators.newLine;
    }
  }
  return attributes;
};
